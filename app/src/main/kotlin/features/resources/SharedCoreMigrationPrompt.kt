...
internal fun SharedCoreMigrationPrompt() {
    ...
    val marker = remember(context) { File(context.noBackupFilesDir, "shared-mihomo-core-migration-v1") }
    ...
    LaunchedEffect(marker, services.resourceFileUseCase) {
        ...
        try {
            if (!withContext(Dispatchers.IO) { marker.exists() }) {
                if (services.resourceFileUseCase.hasCustomMihomoCore()) migration.show = true
