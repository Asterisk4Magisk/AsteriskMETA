...
import engine.root.RootMihomoUid
...
internal class Bpf2SocksRootRunner(
    ...
    modeName = "BPF2SOCKS",
    runtimeConfigTag = "$Bpf2SocksRuntimeMarkerKey:",
    logTag = LogTag,
) {
    ...
    override fun buildPostCoreStartCommand(config: Bpf2SocksStartConfig): String {
        ...
        return buildString {
            appendScript(
                ...
                $${setuidgid.shellQuote()} $${RootMihomoUid.toString().shellQuote()} $${RootMihomoGid.toString().shellQuote()} $${helper.executablePath.shellQuote()} --start --config $${helper.configPath.shellQuote()} --pid $${helper.pidPath.shellQuote()} >> $${config.root.coreLogPaths.errorLogPath.shellQuote()} 2>&1 < /dev/null &
    ...
    override fun buildReadinessCheck(config: Bpf2SocksStartConfig): RootReadinessCheck {
        ...
        return RootReadinessCheck(
            ...
            failureMessage = "mihomo started but bpf2socks bridge port $bridgePort is not ready",
        )
    }
    ...
    override fun StringBuilder.appendStartupFailureDiagnostics(config: Bpf2SocksStartConfig) {
        ...
        appendScript(
            $$"""
                ...
                $${config.root.setuidgidPath.shellQuote()} $${RootMihomoUid.toString().shellQuote()} $${RootMihomoGid.toString().shellQuote()} $${config.controlPaths.executablePath.shellQuote()} --probe --config $${config.controlPaths.configPath.shellQuote()} 2>&1 || true
                echo
    ...
    return buildString {
        appendScript(
            ...
            for bpf2socks_pid in $bpf2socks_pid_candidates; do
                ...
                set -- $uid_line
                [ "$3" = "$${RootMihomoUid}" ] || [ "$5" = "$${RootMihomoUid}" ] || continue
                gid_line="$(grep '^Gid:' "$bpf2socks_pid_dir/status" 2>/dev/null || true)"
                set -- $gid_line
                [ "$3" = "$${RootMihomoGid}" ] || [ "$5" = "$${RootMihomoGid}" ] || continue
    ...
    return buildString {
        appendScript(
            ...
            (
              ...
              set -- $uid_line
              [ "$3" = "$${RootMihomoUid}" ] || [ "$5" = "$${RootMihomoUid}" ] || exit 1
              gid_line="$(grep '^Gid:' /proc/"$pid"/status 2>/dev/null || true)"
              set -- $gid_line
              [ "$3" = "$${RootMihomoGid}" ] || [ "$5" = "$${RootMihomoGid}" ] || exit 1
            )
            """,
        )
    }
