...
internal abstract class RootModeRunner<Config : RootModeStartConfig>(
    ...
    suspend fun start(config: Config) = withContext(Dispatchers.IO) {
        ...
        if (!isRunning(config.root.runtimeLayout)) {
            failStartup(config, "mihomo process exited or did not match the expected $modeName runtime state")
        }
        ...
        runRootCommand(
            config.root.runtimeLayout.buildRepairRuntimePermissionsCommand() +
    ...
    suspend fun prepareCoreLogFiles(coreLogPaths: MihomoCoreLogPaths) = withContext(Dispatchers.IO) {
        val command = coreLogPaths.buildPrepareCoreLogFilesCommand()
        if (command.isBlank()) {
            return@withContext
        }
        runRootCommand(command, "Failed to prepare Mihomo log files")
    }
        ...
        val command = buildString {
            ...
            append(
                buildRootProcessMatchCommand(
                    ...
                    executablePath = runtimeLayout.mihomoCorePath,
                    uid = RootMihomoUid,
                    gid = RootMihomoGid,
                ),
            )
    ...
    private suspend fun failStartup(config: Config, message: String): Nothing {
        ...
        runRootCommand(buildStopCommand(config.root.runtimeLayout), "Failed to clean up after mihomo startup failure")
        error(
            buildRootDiagnosticMessage(
                message,
                processDiagnostics,
