...
/** Indents a complete shell fragment before embedding it in a compound command. */
...
internal fun StringBuilder.appendRootFakeIpIcmpReplyRules(
    ipv4Pool: String,
    ...
    val pool = ipv4Pool.shellQuote()
    appendScript(
        """
        $RootIptablesCommand -t nat -N $RootFakeIpIcmpReplyChain 2>/dev/null || true
        $RootIptablesCommand -t nat -N $RootFakeIpIcmpReplyPreroutingChain 2>/dev/null || true
        $RootIptablesCommand -t nat -I OUTPUT 1 -j $RootFakeIpIcmpReplyChain
        $RootIptablesCommand -t nat -I PREROUTING 1 -j $RootFakeIpIcmpReplyPreroutingChain
        $RootIptablesCommand -t nat -A $RootFakeIpIcmpReplyChain -d $pool -p icmp --icmp-type echo-request -j REDIRECT
        $RootIptablesCommand -t nat -A $RootFakeIpIcmpReplyPreroutingChain -d $pool -p icmp --icmp-type echo-request -j REDIRECT
        """,
    )
}

internal fun StringBuilder.appendRootFakeIpIcmpReplyCleanupRules() {
    appendDeleteRuleLoop(RootIptablesCommand, "OUTPUT", "-j $RootFakeIpIcmpReplyChain", table = "nat")
    appendDeleteRuleLoop(RootIptablesCommand, "PREROUTING", "-j $RootFakeIpIcmpReplyPreroutingChain", table = "nat")
    appendScript(
        """
        $RootIptablesCommand -t nat -F $RootFakeIpIcmpReplyChain 2>/dev/null || true
        $RootIptablesCommand -t nat -X $RootFakeIpIcmpReplyChain 2>/dev/null || true
        $RootIptablesCommand -t nat -F $RootFakeIpIcmpReplyPreroutingChain 2>/dev/null || true
        $RootIptablesCommand -t nat -X $RootFakeIpIcmpReplyPreroutingChain 2>/dev/null || true
        """,
    )
}
