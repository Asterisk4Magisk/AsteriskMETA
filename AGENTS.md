# AsteriskMETA Development Guidelines
...
- When changing the `asteriskd` protocol, configuration schema, resource names, or native source, also check the AsteriskBOX and AsteriskNG submodule versions, sources, and downstream Kotlin adapters so all three contracts remain aligned.
...
- Follow the existing Kotlin, Compose, Gradle Kotlin DSL, C, and Go styles. Keep names explicit, functions focused, and dependency direction clear.
